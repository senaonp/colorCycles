class ColorCycles {
	...
	startColors(): void {
		...
		for (let i: number=0; i<this.elemQueries.length; i+=1) {
			let elems: NodeListOf<HTMLElement> = document.querySelectorAll(this.elemQueries[i]);
...
let myCustomFont3: ColorCycles = new ColorCycles({
...
let randomColorsCycle: ColorCycles = new ColorCycles({
...
let multiElementColorsCycle: ColorCycles = new ColorCycles({
